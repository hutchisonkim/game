...
/// </summary>
public sealed class ChessRules : IPolicy<ChessBoard_Old, BaseMove>
{
    public IEnumerable<BaseMove> GetAvailableActions(ChessBoard_Old state)
    {
    ...
    private static IEnumerable<BaseMove> GetMovesForPiece(ChessBoard_Old state, Position from, Piece piece)
    {
    ...
    private static IEnumerable<BaseMove> PatternMoves(ChessBoard_Old state, Position from, Piece piece, IMovePattern pattern)
    {
        foreach (var (dr, dc) in pattern.GetVectors())
        {
            ...
            while (r >= 0 && r < 8 && c >= 0 && c < 8)
            {
                ...
                {
                    if (target == null)
                        yield return new BaseMove(from, to);
                    else
                        break; // blocked
                }
    ...
    private static IEnumerable<BaseMove> PawnMoves(ChessBoard_Old state, Position from, Piece piece)
    {
        ...
        {
            yield return new BaseMove(from, oneFwd);
            ...
            if (from.Row == startRow && twoFwd.IsValid && state.PieceAt(twoFwd) == null)
                yield return new BaseMove(from, twoFwd);
        }
        ...
        if (diagLeft.IsValid)
        {
            ...
            if (t != null && t.Color != piece.Color)
                yield return new BaseMove(from, diagLeft);
        }
        if (diagRight.IsValid)
        {
            var t = state.PieceAt(diagRight);
            if (t != null && t.Color != piece.Color)
                yield return new BaseMove(from, diagRight);
        }
    }
